...
For container deployments, keep scheduling in host cron/systemd and run one-shot compose services:

```bash
docker compose run --rm sync-bookmarks
docker compose run --rm sync-likes
docker compose --profile examples run --rm scheduled-sync-example
```
